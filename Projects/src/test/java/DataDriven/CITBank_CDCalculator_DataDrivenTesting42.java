...
public class CITBank_CDCalculator_DataDrivenTesting42 {
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		int rows=Excel.getRowCount(path, "Sheet1");				// get a row count from the sheet
		System.out.println("row count is : " + rows);
		
		for(int i=1;i<=rows;i++)
		{			
			//Reading data from excel
			String inidepo= Excel.getCellData(path,"Sheet1",i,0);				//file,sheet,row #, col zero for initial deposit
			String interestrate=Excel.getCellData(path,"Sheet1",i, 1);		//interest rate
			String monthlength=Excel.getCellData(path,"Sheet1",i, 2);			//length
			String compoundingmonths=Excel.getCellData(path,"Sheet1",i, 3);	//compounding
			String exptotal=Excel.getCellData(path,"Sheet1",i, 4);	//expected total , will be compared with actual total
			...
			if(exptotal.equals(acttotal)) {			//if expected total = actual total then			
				...
				Excel.setCellData(path, "Sheet1",i, 6,"Passed");	//setting passed in 6th column (index start with zero)
				Excel.fillGreenColor(path, "Sheet1",i, 6);	//filling the color in 6th column if passed then greeen or faile then red.
			}
			else
			{
				Excel.setCellData(path, "Sheet1",i, 6,"Failed");
				Excel.fillRedColor(path, "Sheet1",i, 6);
			}
		}
